...
class CodeIntelFlowClientConfig(ClientConfig):
    def __init__(self):
        self.name = "flow"
        self.binary_args = [
            node_command(),
...
class CodeIntelFlowPlugin(LanguageHandler):
    def __init__(self):
        self._server_name = "Flow Language Server"
        self._config = CodeIntelFlowClientConfig()
    ...
    def on_diagnostics(self, params):
        spinner.start("Flow-CodeIntel", spinner='monkey')
